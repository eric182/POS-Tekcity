...
function get_person_data_row($person,$controller)
{
	...
	if ($controller_name =='customers') {
		$table_data_row.='<td width="20%">'.character_limiter($person->account_number,13).'</td>';
	} else {
		$table_data_row.='<td width="20%"></td>';
	}	
...
function get_expenses_manage_table_data_rows($expenses,$controller)
{
	...
	{
		$table_data_rows.=get_expense_data_row($expense,$controller);
	}
	
	if($expenses->num_rows()==0)
	{
		$table_data_rows.="<tr><td colspan='6'><div class='warning_message' style='padding:7px;'>".$CI->lang->line('common_no_records_to_display')."</div></tr></tr>";
	}
...
function get_expense_data_row($expense,$controller)
{
	...
	$table_data_row='<tr>';
	$table_data_row.="<td width='5%'><input type='checkbox' id='expense_$expense->expense_id' value='".$expense->expense_id."'/></td>";
	$table_data_row.='<td width="17%">'.character_limiter($expense->expense_date,13).'</td>';
	$table_data_row.='<td width="17%">'.character_limiter($expense->description,13).'</td>';
	$table_data_row.='<td width="17%">'.character_limiter(number_format($expense->amount,2),13).'</td>';
	$table_data_row.='<td width="5%">'.anchor($controller_name."/view/$expense->expense_id/width:$width", $CI->lang->line('common_edit'),array('class'=>'thickbox','title'=>$CI->lang->line($controller_name.'_update'))).'</td>';		
	$table_data_row.='</tr>';
