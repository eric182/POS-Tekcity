...
function post_expenses_form_submit(response)		 
{
	if(!response.success)
	{
	...
	else
	{
		...
		if(jQuery.inArray(response.expense_id,get_visible_checkbox_ids()) != -1)
		{
			update_row(response.expense_id,'<?php echo site_url("$controller_name/get_row")?>');
		...
		else //refresh entire table
		{
			...
			{
				//highlight new row
				hightlight_row(response.expense_id);
				set_feedback(response.message,'success_message',false);		
			});
		}
	}
...
<div id="title_bar">
	...
	<div id="new_button">
		...
		?>
		<?php //if ($controller_name =='expenses') {?>
